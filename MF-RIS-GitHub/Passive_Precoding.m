function [R] = Passive_Precoding(N,K,M,sigma1,Theta,f_k,h_k,g_k,H)
iteration = 40;
obj = zeros(2*iteration, 1);
nu = ones(K, 1);
for i = 1:iteration
    mu = Update_mu(M,N,K,h_k,g_k,H,Theta,nu,f_k); 
    nu = STAR_Passive_Update_nu(K,N,M,mu,h_k,g_k,H,Theta,f_k,sigma1); 
    [~,obj(2*i-1)] = STAR_Passive_Update_SINR(K,N,M,h_k,g_k,H,Theta,f_k,sigma1);
    [v,Q] = STAR_Passive_Update_Q_v(K,N,M,mu,nu,h_k,g_k,H,f_k);
    theta_temp1 = Passive_Update_theta(M,v,Q); 
