function [nu,Lam] = MF_Active_Update_Q_v(K,N,M,Rho_k,eps_k,h_k,g_k,H,f_k,sigma2)
...
for k=1:K 
    ...
    Lam_k(k,:,:)=abs(eps_k(k))^2*diag(abs(temp_g_k).^2)*sigma2+abs(eps_k(k))^2*diag(temp_g_k')*H*temp2*H'*diag(temp_g_k);
end
