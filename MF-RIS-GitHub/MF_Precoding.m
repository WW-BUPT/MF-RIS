function [R] = MF_Precoding(N,K,M,PRIS,sigma1,sigma2,MF_Theta,f_k,h_k,g_k,H,beta_max)
iteration = 40;
obj = zeros(2*iteration, 1);
nu = ones(K, 1);
for i = 1:iteration
    mu = Update_mu(M,N,K,h_k,g_k,H,MF_Theta,nu,f_k); 
    nu = MF_Active_Update_nu(K,N,M,mu,h_k,g_k,H,MF_Theta,f_k,sigma1,sigma2); 
    [~,obj(2*i-1)] = MF_Active_Update_SINR(K,N,M,h_k,g_k,H,MF_Theta,f_k,sigma1,sigma2);
    [v,Q] = MF_Active_Update_Q_v(K,N,M,mu,nu,h_k,g_k,H,f_k,sigma2);
    theta_temp1 = MF_Update_theta(M,K,N,v,Q,f_k,H,PRIS,sigma2,beta_max); 
